 ...
 *
 * Ktor-specific capture lives in the `lens-ktor` artifact; OkHttp in `lens-okhttp`.
 */
object LensNetworkLogStore {
    ...
    fun logMockedResponse(
        ...
        val prettyBody = if (rule.body.isBlank()) {
            ...
            try {
                ...
                pretty.encodeToString(JsonElement.serializer(), pretty.parseToJsonElement(rule.body))
            } catch (_: Throwable) {
                rule.body
            }
        }
    ...
    @OptIn(ExperimentalTime::class)
    suspend fun recordResponseSuccess(callId: Int, responseData: ResponseData) {
        mutex.withLock {
            if (callId !in stateCalls.indices) return
            val sendTime = stateCalls[callId].requestStartEpochMs ?: return
    ...
    fun recordOutboundFailure(callId: Int, cause: Throwable?) {
        if (callId !in stateCalls.indices) return
        stateCalls[callId] = stateCalls[callId].copy(
            request = Resource.Failed(stateCalls[callId].requestData, cause),
            response = null,
        )
    }

    fun recordResponseFailure(callId: Int, cause: Throwable?) {
        if (callId !in stateCalls.indices) return
        stateCalls[callId] = stateCalls[callId].copy(
            response = Resource.Failed(stateCalls[callId].responseData, cause),
        )
